#ifndef LINUX_LISTHEAD_H_INCLUDE
#define LINUX_LISTHEAD_H_INCLUDE
...
/**
 * list_for_each_prev_safe   -   iterate over a list backwards against removal of list entry
 * @pos:        the &struct list_head to use as a loop cursor.
 * @p:        	another &struct list_head to use as temporary storage
 * @head:       the head for your list.
 */
...
#ifdef __cplusplus
struct list_head2 : public list_head{
    unsigned int count;		//list count
    unsigned int size;		//list memory size
};

static void inline init_list_head2(list_head2 * ptr)
{
    INIT_LIST_HEAD(ptr);
    ptr->count = 0;
    ptr->size = 0;
...
#endif	/* LINUX_LISTHEAD_H_INCLUDE */
